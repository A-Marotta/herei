...
router.post('/users/register', async (req, res) => {
    try {
        ...
        if (errors.length > 0) {
            ...
            let hashedPassword = await bcrypt.hash(req.body.password, 10); //10 is the convention default amount;
            console.log(hashedPassword);

            pool.query(
                `INSERT INTO users (name, email, password_digest)
                VALUES ($1, $2, $3)
                RETURNING id, password_digest`, [req.body.name, req.body.email, hashedPassword],
                async (err, results) => {
                    if (err) {
                        throw err;
                    }
                    ...
                    try {
                        await sendEmail(`${req.body.email}`, "Verify Email", `Please click <a href="${process.env.BASE_URL}/users/verify?user_email=${req.body.email}">link</a> to confirm.`);
